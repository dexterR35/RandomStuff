...
const config = {
    leverDuration: 400, // 0.5 seconds for lever animation
    wheelDuration: 1000, // seconds for wheel rotation
    leverReturnDuration: 300, // seconds for lever return animation
    delayBetweenWheels: 250, // seconds delay for each wheel
    totalWheelRotation: 2 * Math.PI // 2 * Math.PI (360deg) for wheel rotation
};
...
function init() {
    ...
    loader.load('./roata_good_best.glb', function (gltf) {
        // Set initial position and rotation of the entire model
        // gltf.scene.position.set(0, 0, 0);
        // gltf.scene.rotation.set(0, -0.5, 0); // Set initial rotation of the root object, adjust as needed
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            if (wheel) {
                ...
                wheel.children.forEach((child, index) => {
                    if (child.isMesh) {
                        child.material = new THREE.MeshStandardMaterial({ color: new THREE.Color(`hsl(${index * 200}, 100%, 50%)`) });
                        // console.log(`Set color for ${child.name} of roata${i}`);
                    }
    ...
    function animate(time) {
        ...
        wheels.forEach((wheel, index) => {
            if (wheel && elapsed >= config.delayBetweenWheels * index) {
                ...
                wheel.rotation.x = easeInOutQuad(wheelProgress) * config.totalWheelRotation; // Apply easing and rotation
            }
        });
...
function animateLeverAndWheels() {
    ...
    function animateLever(time) {
        ...
        if (leverProgress < 1) {
            ...
            console.log('Lever animation completed');
...
function disableInteractions() {
    ...
    // document.getElementById('toggleButton').disabled = true; // Optional: Disable toggle button
...
function showWinningMessage() {
    ...
    messageDiv.innerHTML = `
        <div id="winningMessage" style="
            ...
            font-size: 24px;
            text-align: center;">
            Congrats, you win 250 Runde gratuite!
        </div>
    `;
